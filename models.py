...
database = SqliteDatabase('inf.db')
...
class Account(BaseModel):
    ...
    wxid = CharField()
    account = CharField()
    lastSyncTime = IntegerField()
    lastSyncCount = IntegerField()
...
#     def gravatar_url(self, size=79):
#         return 'http://www.gravatar.com/avatar/%s?d=identicon&s=%d' % \
#             (md4(self.email.strip().lower().encode('utf-8')).hexdigest(), size)
...
#     user = ForeignKeyField(User)
#     message = TextField()
#     status = IntegerField(choices=((0, 'live'), (2, 'deleted')), null=True)
